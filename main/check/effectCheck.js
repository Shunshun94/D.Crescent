...
io.github.shunshun94.HiyokoCross.EffectCheck = class extends com.hiyoko.component.ApplicationBase {
	...
	buildComponents () {
		...
		this.getElementsByClass(`effect-name`).each((i, $dom) => {
			$($dom).text(this.effects[i].name);
		});
		this.getElementsByClass(`effect-note`).each((i, $dom) => {
			$($dom).text(this.effects[i].notes || '');
		});
		this.getElementsByClass(`effect-spec-type`).each((i, $dom) => {
			$($dom).text(this.effects[i].type || '');
		});
		this.$html.append(`<button id="${this.id}-exec">エフェ クト行使 (判定は行われず侵蝕率のみ上昇します)</button>`);
	}
	...
	generateEffectDom (skill, num) {
		return 	`<div class="${this.id}-effect ${this.clazz}-skill" id="${this.id}-skill-${num}">` +
				...
				`<span class="${this.id}-effect-name ${this.clazz}-skill-name"></span>` +
